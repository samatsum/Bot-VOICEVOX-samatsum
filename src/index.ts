...
async function main(){
  ...
  if(!lock.ok){const age=Math.max(0,Math.round((Date.now()-lock.heartbeatAt)/1000));console.error(`Another Bot-VOICEVOX-samatsum instance is active. instance=${lock.activeInstanceId}, heartbeat=${age}s ago`);console.error(`If the other PC crashed, wait about ${Math.ceil(env.instanceLockTtlMs/1000)} seconds and try again.`);await closeDatabase().catch(()=>undefined);process.exitCode=2;return;}
  ...
  client.once(Events.ClientReady,async(readyClient)=>{
    ...
    console.log('Bot-VOICEVOX-samatsum is ready: dictionary, settings, status, and VC lifecycle are available');
    ...
    try{
      ...
      if(interaction.isChatInputCommand()){
        ...
        if(!ALWAYS_AVAILABLE_COMMANDS.has(interaction.commandName)){
          if(!interaction.inGuild() || !(await canUseBotByPermissions(interaction.guildId,interaction.memberPermissions))){await interaction.reply({content:'このサーバーのBot-VOICEVOX-samatsumは現在 **admin** モードです。「サーバー管理」権限を持つユーザーだけが利用できます。',flags:MessageFlags.Ephemeral});return;}
        }
